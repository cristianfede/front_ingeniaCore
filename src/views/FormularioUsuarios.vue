      ...
      <v-card-text>
        <v-form @submit.prevent="onSubmit" class="form" style="color: black">
          ...
          <div class="d-flex justify-start">
            <v-btn v-if="isEditing" color="secondary" @click="resetFormAndState" class="mr-2"
              >Cancelar Edición</v-btn
            >

            <v-btn v-if="!isEditing" color="grey" text @click="resetFormAndState" class="mr-2"
...
const { value: apellido, errorMessage: apellidoError } = useField('apellido', [
  required,
  (val: string) => minLength(val, 2),
  onlyLetters,
])
const { value: telefono, errorMessage: telefonoError } = useField('telefono', [
  required,
  isPhoneNumber,
])
...
const { value: password, errorMessage: passwordError } = useField('password', (value: string) => {
  if (!isEditing.value) {
    ...
    if (value && value.length < 6)
      return 'La contraseña debe tener al menos 6 caracteres si se proporciona.'
  }
  return true
})
...
const {
  value: empresa,
  errorMessage: empresaError,
  handleBlur: empresaBlur,
} = useField('empresa', [required])

// --- Estados adicionales del formulario y datos ---
const roles = ref<Array<{ id: number; nombre: string }>>([])
const empresas = ref<Array<{ id: number; nombre: string }>>([])
...
interface Usuario {
  ...
  telefono: string
  correo: string
  rol?: { id: number; nombre: string } // Added 'rol' property
  empresa?: { id: number; nombre: string } // Added 'empresa' property
  rol_display_name: string
  empresa_display_name: string
}
...
async function fetchEmpresas() {
  try {
    ...
    snackbar.value = {
      show: true,
      message: `Error al cargar las empresas: ${errorMessage}`,
      color: 'error',
    }
  }
}
...
async function fetchRoles() {
  try {
    ...
    snackbar.value = {
      show: true,
      message: `Error al cargar los roles: ${errorMessage}`,
      color: 'error',
    }
  }
}
...
async function cargarUsuarios() {
  try {
    ...
    usuarios.value = fetchedUsers.map((user: Usuario) => {
...
function editUser(user: Usuario) {
  ...
  setValues({
    ...
    empresa: user.empresa ? user.empresa.id : null,
  })
  password.value = ''
}
  ...
  try {
    if (currentAction.value === 'create') {
      const userData = {
        ...
        rol_id: rol.value as number, // Explicitly cast to number
        empresa_id: empresa.value as number, // Explicitly cast to number
      }
      ...
      const userDataToUpdate: UserDataToUpdate = {
        ...
        telefono: telefono.value,
        correo: correo.value,
        rol_id: rol.value !== undefined ? Number(rol.value) : null,
        empresa_id: empresa.value ? Number(empresa.value) : null,
      }
      if (password.value) {
        userDataToUpdate.password = password.value
      }
      console.log(
        'DEBUG: Actualizando usuario con ID:',
        editingUserId.value,
        ' y datos:',
        userDataToUpdate,
      )
    ...
    const errorMessage =
      (err instanceof Error && err.message) || 'Error desconocido al procesar la operación.'
    if ((err as { response?: { data?: { errors?: Array<{ field: string; message: string }> } } })?.response?.data?.errors) {
      const backendErrors = (err as { response?: { data?: { errors?: Array<{ field: string; message: string }> } } }).response?.data?.errors ?? []
        .map((e: { field: string; message: string }) => `${e.field}: ${e.message}`)
        .join(', ')
      snackbar.value = {
        show: true,
        message: `Error de validación: ${backendErrors}`,
        color: 'error',
      }
    } else {
