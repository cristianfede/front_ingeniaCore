      ...
      <v-card-text>
        ...
        <v-form ref="form" @submit.prevent="submitForm" class="form" style="color: black">
          ...
          <div class="d-flex justify-start">
            <v-btn v-if="isEditing" color="secondary" @click="resetForm" class="mr-2"
              >Cancelar Edición</v-btn
            >
            <v-btn v-if="!isEditing" color="grey" text @click="resetForm" class="mr-2"
...
import {
  ...
  crearRol,
  actualizarRol,
  eliminarRolPermanentemente,
  verificarNombreRolUnico,
  inactivarRol,
  activarRol,
} from '../services/CreacionRoles' // <-- Importa verificarNombreRolUnico y las funciones de estado si las usas directamente
...
const estado = ref<'activo' | 'inactivo'>('activo') // Estado por defecto para la creación
...
const rules = {
  ...
  descriptionMaxLength: (v: string) =>
    (v && v.length <= 255) || 'Máximo 255 caracteres para la descripción.',
...
async function cargarRoles() {
  try {
    ...
    roles.value = fetchedRoles.map((rol) => ({
      id: rol.id,
      nombre: rol.nombre,
      descripcion: rol.descripcion ?? '', // Ensure descripcion is always a string
      estado: rol.estado,
    }))
  } catch (error: unknown) {
...
async function submitForm() {
  ...
  if (!valid) {
    snackbar.value = {
      show: true,
      message: 'Por favor, corrige los errores del formulario antes de continuar.',
      color: 'warning',
    }
...
async function handleConfirmAction() {
  ...
  try {
    const rolData: { nombre: string; descripcion?: string | null; estado?: 'activo' | 'inactivo' } =
      {
        nombre: nombre.value,
        descripcion: descripcion.value || null, // Ensure descripcion is null if empty
      }
    ...
    const errorMessage =
      ...
      'Error al procesar la operación del rol. Intenta de nuevo más tarde.'
    ...
    if (errorMessage.includes('El nombre del rol ya está en uso.')) {
      // Compara con el mensaje que devuelve tu controlador en caso de conflicto
      formAlertMessage.value =
        'El nombre de rol ingresado ya está en uso. Por favor, elige uno diferente.'
...
function editRol(rol: Role) {
  isEditing.value = true
  editingRolId.value = rol.id
  nombre.value = rol.nombre
  descripcion.value = rol.descripcion
  estado.value = rol.estado === 'activo' || rol.estado === 'inactivo' ? rol.estado : 'activo'
...
function handleDeleteRol(id: number) {
  ...
  confirmDialogMessage.value =
    '¿Estás seguro de que quieres inactivar este rol? Se cambiará su estado a inactivo.'
...
function handleDeletePermanentlyRol(id: number) {
  ...
  confirmDialogMessage.value =
    'Esta acción eliminará el rol de forma definitiva de la base de datos y no se podrá recuperar. ¿Estás absolutamente seguro?'
...
function handleActivateRol(id: number) {
  ...
  confirmDialogMessage.value =
    '¿Estás seguro de que quieres activar este rol? Se cambiará su estado a activo.'
...
const filteredRoles = computed(() => {
  ...
  if (sortBy.value && sortBy.value.length > 0) {
    ...
    rolesFiltrados.sort((a, b) => {
      let valA = a[sortKey as keyof Role]
      let valB = b[sortKey as keyof Role]
