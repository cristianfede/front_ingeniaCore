<template>
  <v-toolbar color="#2962FF" dark app elevation="2">
    ...
    <v-toolbar-title> Bienvenido, {{ authStore.user?.nombre || 'Usuario' }} </v-toolbar-title>

    <v-spacer></v-spacer>

    <v-menu location="bottom end" offset-y>
      <template #activator="{ props }">
        <v-badge :content="authStore.cantidadNoLeidas" color="red" v-bind="props" overlap>
      ...
      <v-list>
        ...
        <v-list-item @click="router.push({ name: 'Notificaciones' })">
          <v-list-item-title class="text-primary"> Ver todas las notificaciones </v-list-item-title>
        </v-list-item>
      </v-list>
    </v-menu>
...
async function irADetalle(noti: { id: number; ticketId: number; titulo: string; mensaje: string }) {
